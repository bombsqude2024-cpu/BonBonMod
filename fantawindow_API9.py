...
quick_msg_file = _bui.env()['python_directory_user'] + '/QuickMessages.txt'
...
class PartyWindow(bui.Window):
    ...
    def __del__(self) -> None:
        _bui.set_party_window_open(False)

    def __init__(self, origin: Sequence[float] = (0, 0)):
        self.ping_server()
        _bui.set_party_window_open(True)
        self._r = 'partyWindow'
        ...
        self.ping_timer = bui.Timer(5, bui.WeakCall(self.ping_server), repeat=True)

        bui.Window.__init__(self, root_widget=bui.containerwidget(
            size=(self._width, self._height),
            transition='in_scale',
            color=self.bg_color,
            parent=_bui.get_special_widget('overlay_stack'),
        ...
        info = _bui.get_connection_to_host_info()
        if info.get('name', '') != '':
        ...
        self._update_timer = bui.Timer(1.0,
    ...
    def _on_party_member_press(self, client_id: int, is_host: bool,
        ...
        if _bui.get_foreground_host_session() is not None:
    ...
    def popup_menu_selected_choice(self, popup_window: PopupMenuWindow,
        ...
        if self._popup_type == 'partyMemberPress':
            if choice == 'kick':
                ...
                cur_msg = bui.textwidget(query=self._text_field)
                if cur_msg.endswith(' '):
                    self._edit_text_msg_box(player_names)
                else:
            ...
            if choice == 'muteChat':
                ...
                try:
                    ...
                    bui.playsound(bui.getsound('dingSmallHigh'))
                except:
                    bui.print_exception()
            ...
            bui.playsound(bui.getsound('shieldDown'))
    ...
    def _send_chat_message(self) -> None:
        ...
        if msg == '/save':
          ...
          if info.get('name', '') != '':
            ...
            bui.playsound(bui.getsound('gunCocking'))
            bui.textwidget(edit=self._text_field,text="")
            return
            ...
            if replied_msg != '':
                ...
                _bui.chatmessage('          | ' + replied_msg) 
            #_bui.chatmessage(cast(str, msg))
            msg_parts = split_msg(cast(str, msg))
            for part in msg_parts:
                _bui.chatmessage(part)
    ...
    def _send_ping(self):
        if isinstance(_ping, int):
            _bui.chatmessage(f'my ping: {_ping}ms')
        ...
        for index, choice in enumerate(choices):
            ...
            if self._use_scroll:
                self._width = max(
                    ...
                    min(
                        maxwidth,
                        _bui.get_string_width(choice_display_name,
...
class PingThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        try:
            import socket
            from bui.internal import get_ip_address_type
            ...
            bui.print_exception('Error on gather ping', once=True)
        finally:
            try:
                if sock is not None:
                    sock.close()
            except Exception:
                bui.print_exception('Error on gather ping cleanup', once=True)
...
class InitalRun(babase.Plugin):
    def __init__(self):
        if _bui.env().get("build_number",0) >= 20918:
            #_bui.connect_to_party = modify_connect_to_party
